...
{
  return CDBusUtil::GetAll(destination, object, "org.freedesktop.DBus.Properties", "Get", arg);
}
...
CVariant CDBusUtil::ParseType(DBusMessageIter *itr)
{
  ...
  case DBUS_TYPE_ARRAY:
    DBusMessageIter arrayContainer;
    dbus_message_iter_recurse(itr, &arrayContainer);
    DBusMessageIter arrayContent;
    value = CVariant::VariantTypeArray;
    do
    {
      dbus_message_iter_recurse(&arrayContainer, &arrayContent);
      CVariant item = ParseDictionary(itr);
      if (!item.isNull())
      {
        value.push_back(item);
      }
    } while (dbus_message_iter_has_next(itr));
    break;
